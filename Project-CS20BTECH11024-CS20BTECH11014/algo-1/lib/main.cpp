...
void updtMem(Sch *Scheduler, int ThId) {
  ...
  for (int CurrTrans = 0; CurrTrans < NumTrans; CurrTrans++) {
    ...
    do {
      ...
      Transaction *T = Scheduler->begin_trans();
      int TransId = T->Id;
      int RandIters = 1 + (rand() % (M_DataItems / 2));
      int LocalVal = -1;
      ...
      for (int I = 0; I < RandIters; I++) {
        ...
        LocalVal = Scheduler->invoke(T, Operation::READ, RandInd, LocalVal);
        ...
        LocalVal += RandVal;
        // Scheduler->write(T, RandInd, LocalVal);
        Scheduler->invoke(T, Operation::WRITE, RandInd, LocalVal);
...
void init(const std::string &LogFileName) {
  ...
  Sch *Scheduler = new Sch(&SharedMemory, LogFile);
  std::thread Threads[N];
  for (int I = 0; I < N; I++)
    Threads[I] =
        std::thread(updtMem<Sch, Transaction, DataItemInfo>, Scheduler, I);
...
int main(int argc, char *argv[]) {
  ...
  init<Scheduler, Transaction, DataItemInfo>("Scheduler-log.txt");
  return 0;
}
