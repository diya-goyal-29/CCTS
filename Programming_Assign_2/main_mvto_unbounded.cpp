...
int abortCount = 0;
data_head* Memory;
std::string name = "MVTO_unbounded_log_file.txt";


void uptMem(MVTO_unbounded* MVTO_unboundedSch, FILE *file, int threadId);
int getSysTime();
...
int main(int argc, char* argv[]) {
	...
	MVTO_unbounded MVTO_unboundedSch;
...
void uptMem(MVTO_unbounded* MVTO_unboundedSch, FILE *file, int threadId) {
	...
	for (int curTrans = 0; curTrans < numTrans; curTrans++) {
		...
		double CommitDelay = (critEnd - critStart)/(float) CLOCKS_PER_SEC * pow(10,3);
		avgTime += CommitDelay;
		abortCount += abortCnt;
	}
}